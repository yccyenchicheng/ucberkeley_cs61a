...
class Place(object):
    ...
    def add_insect(self, insect):
        ...
        if insect.is_ant:
            if self.ant is None:
                ...
                # BEGIN Problem 11
                cur_ant = self.ant
                if cur_ant.can_contain(insect):
                    cur_ant.contain_ant(insect)

                elif insect.can_contain(cur_ant):
    ...
    def remove_insect(self, insect):
        ...
        """
        if hasattr(insect, 'name') and insect.name == 'Queen' and insect.impostor == False:
            pass
        else:    
            if insect.is_ant:
...
class Bee(Insect):
    ...
    def blocked(self):
        ...
        # BEGIN Problem 8
        ant_on_path = self.place.ant
        if ant_on_path:
            if ant_on_path.blocks_path:
                return True
            else:
                return False
        return False
...
class Ant(Insect):
    ...
    def can_contain(self, other):
        # can_contain only return True if:
        # a. it's a container
        # b. other ant is not a container
        # c. it current do not contain any ant
        return self.container and other.container == False and self.ant == None
...
class ThrowerAnt(Ant):
    ...
    def nearest_bee(self, hive, min_range=min_range, max_range=max_range):
        ...
        # count_range is used to keep track of distance of the place we're inspecting 
        count_range = 0
        cur_place = self.place

        # count_range is only effective when it's between min_range and max_range
        while count_range < min_range:
            count_range += 1
            cur_place = cur_place.entrance

        # We cannot attack the bees in the hive
        while count_range <= max_range and cur_place != None and cur_place != hive:
            if cur_place.bees:
...
class LongThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        return ThrowerAnt.nearest_bee(self, hive, self.min_range, ThrowerAnt.max_range)
    # END Problem 6
...
class ShortThrower(ThrowerAnt):
    ...
    def nearest_bee(self, hive):
        return ThrowerAnt.nearest_bee(self, hive, ThrowerAnt.min_range, self.max_range)
    # END Problem 6


# BEGIN Problem 7
class WallAnt(Ant):
    """WallAnt does nothing but has lots of armor and cost lots of food."""
...
class QueenAnt(ScubaThrower):  # You should change this line
    ...
    def action(self, colony):
        ...
        if self.impostor:
            ...
            while back_place != None:
                back_ant = back_place.ant
                if isinstance(back_ant, Ant):
                    if back_ant not in self.buffed_ants:
                        if back_ant.container and back_ant.ant:
                            if back_ant.ant not in self.buffed_ants:
                                back_ant.damage *= 2    
                                self.buffed_ants += [back_ant]
                        else:
                            back_ant.damage *= 2    
                            self.buffed_ants += [back_ant]

                    if back_ant in self.buffed_ants and back_ant.container:
                        if back_ant.ant and back_ant.ant not in self.buffed_ants:
                            back_ant.ant.damage *= 2
                            self.buffed_ants += [back_ant.ant]
    ...
    def reduce_armor(self, amount):
        ...
        if self.impostor == False and self.armor <= 0:
            bees_win()
        # END Problem 13
