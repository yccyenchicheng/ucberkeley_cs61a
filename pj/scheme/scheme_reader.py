...
def scheme_read(src):
    ...
    if val == 'nil':
        ...
        # Return a Pair that wraps the Scheme expression
...
def read_tail(src):
    ...
    try:
        if src.current() is None:
            ...
            # Continue to read expression 
            val = scheme_read(src)
            if src.current() == ')':

                # Removing the closing parenthesis. And then return what we have read
                src.remove_front()
                return val
            else:

                # If there's no closing parenthesis, then it must be an error
                raise SyntaxError
